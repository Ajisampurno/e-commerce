  ...
  <div class="container mx-auto">
      ...
      <div class="flex justify-between items-center">
        ...
        <nav>
          ...
          <ul class="flex space-x-4 items-center">
            ...
            <li><router-link to="/about" class="text-blue-500 hover:text-blue-700">Information</router-link></li>
                  ...
                  <ul>
                    <li v-for="(item, index) in cart" :key="item.id" class="flex justify-between items-center mb-2">
                      <div class="flex-1">
                        <span>{{ item.name }}</span>
                        <br />
                        <span class="text-gray-500 text-sm">Variant: {{ item.selectedVariant.value }}</span>
                        <br />
    ...
    <main class="py-8">
      ...
      <section class="max-w-6xl mx-auto">
        ...
        <!-- Product Image -->
        <div class="my-5">
            <img :src="product.photo" :alt="product.name" class="object-cover object-center rounded-lg shadow-md" />
        </div>
        <hr>
        <p>{{ product.page_body }} Lorem ipsum dolor sit amet consectetur adipisicing elit. Deserunt dolor exercitationem consequatur, eius inventore eaque perspiciatis unde pariatur veniam quo fuga adipisci totam temporibus corrupti error? Quos vero ullam sapiente atque. Assumenda, libero voluptatem labore velit non, perferendis sed quae cumque, dolores totam a laboriosam reprehenderit itaque voluptatibus fugiat doloremque et. Voluptate aperiam, commodi placeat veniam, mollitia vel nostrum nam voluptas voluptatum labore, quos sint! Unde iste iusto, inventore amet rerum facere ut eaque? Harum voluptas, veritatis sequi quas porro minus veniam cupiditate dolore eos impedit aliquam reprehenderit, quibusdam iusto voluptatum. Pariatur, eius fugiat veritatis consequuntur suscipit impedit enim officiis.</p>
      </section>
    </main>
  ...
  methods: {
    async fetchProduct() {
      try {
        ...
        const response = await axios.get(`https://demo.sistemtoko.com/public/demo/article/${productId}`)
        this.product = {
          id: response.data.id,
    ...
    addToCart() {
      const existingCartItem = this.cart.find(item => item.id === this.product.id && item.selectedVariant.value === this.selectedVariant.value)
      if (existingCartItem) {
